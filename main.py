...
if __name__ =="__main__":
    ...
    minist = datasets.CIFAR10(root='./data/',train = True,transform = img_transform,download=True)

    trainloader = torch.utils.data.DataLoader(
        dataset = minist,batch_size = 256,shuffle =True,
        drop_last = True
    )
    '''
    ...
    #C = Net()
    C = ResNet18()#LogisticRegression(784,10)
    #C.load_state_dict(torch.load("./class.pth"))
    C.load_state_dict(torch.load("./class.pth"))
    g_optimizer = torch.optim.Adam(G.parameters(),lr = 0.001)
    d_optimizer = torch.optim.Adam(D.parameters(),lr = 0.001)

    train_2(d_optimizer=d_optimizer,g_optimizer = g_optimizer,
          dataloader = trainloader, epoch_num = epoch_num,
          G=G,D=D,C=C,criterion = criterion)
